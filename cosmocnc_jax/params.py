...
cnc_params_default = {
    ...
    "tpu_shard": 0, #>1: shard one eval's abundance patch axis over that many local devices (GSPMD; exact padding). 0=off
    "tpu_shard_bc": False, #also shard the bc cluster axis (measured counterproductive on v6e-4: collectives dominate); default off
...
# and planck_cosmology/build_wl_prior_magneticum.py). The joint Gaussian
# prior replaces the previous single-nuisance `lnb_wl_sigma`.
...
# _Z_WL = [0.01, 0.252, 0.470, 0.783]; see planck_cosmology/
# survey_sr_planck_szifi_jax.py.
